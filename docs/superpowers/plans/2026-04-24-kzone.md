...
k-tools/
  ...
  src/
    ...
    components/
      ...
      FavoritesList.jsx      # pinned cities quick-access strip
      FavoritesList.css
      FavoritesList.test.jsx
    features/
      reverse/
        ReverseSearch.jsx    # time picker + date toggle + badge
...
## Task 11: Date and Time Override
...
- [ ] **Step 4: Add `referenceDate` state to `App.jsx`**

Open `src/App.jsx`. Add state and pass it down:

```jsx
const [referenceDate, setReferenceDate] = useState(null); // null = use current time
```

Pass to both feature sections:
...
- [ ] **Step 5: Update `ReverseSearch.jsx` — add datetime toggle**

Replace `ReverseSearch.jsx` entirely:
...
export default function ReverseSearch({
  ...
  function handleDateTimeChange(e) {
    if (!e.target.value) { onReferenceDateChange(null); return; }
    onReferenceDateChange(new Date(e.target.value));
  }

  const dateTimeValue = referenceDate
    ? referenceDate.toISOString().slice(0, 16)
    : '';
  ...
  return (
    <div className="reverse-search">
      ...
      <div className="reverse-search__row">
        <label className="reverse-search__label" htmlFor="reference-date">
          On:
        </label>
        <input
          id="reference-date"
        ...
        {referenceDate && (
          <button
            className="reverse-search__now-btn"
...
Add to `ReverseSearch.css`:

```css
.reverse-search__now-btn {
  padding: var(--space-1) var(--space-3);
...
.reverse-search__now-btn:hover {
  background: var(--color-surface);
}
```

- [ ] **Step 6: Update `ReverseResults.jsx` — use `referenceDate` prop**

Add `referenceDate` prop and pass it through:
...
export default function ReverseResults({ targetHour, homeTimezone, referenceDate }) {
  ...
  // ... rest of JSX unchanged, but pass ref to getDayInTimezone and formatTimeInTimezone calls:
  // getDayInTimezone(city.timezone, ref)
  // formatTimeInTimezone(city.timezone, ref)
```

- [ ] **Step 7: Update `ForwardResult.jsx` — use `referenceDate` prop**

Add `referenceDate` to props and update all `useMemo` calls:
...
Open [http://localhost:5173](http://localhost:5173) and verify:
- The "On:" datetime input is visible; selecting a past or future datetime changes all results
- The "Now" button appears when a custom date is set and resets results to current time
- Forward lookup result also updates when the reference date changes
...
```bash
git add src/
git commit -m "feat: add date/time override for hypothetical past and future lookups"
```
...
export default function FavoritesList({ favorites, onSelect, referenceDate }) {
  ...
  return (
    <div className="favorites-list">
      <span className="favorites-list__label">Favorites</span>
...
Import `useFavorites` and add a star toggle to each result card. The `ReverseResults` component now accepts `favorites`, `addFavorite`, `removeFavorite` as props (passed from App):
...
<button
  ...
  aria-label={isFavorite(city.timezone) ? 'Remove from favorites' : 'Add to favorites'}
>
...
Same pattern — accept `isFavorite`, `addFavorite`, `removeFavorite` as props and add a star button adjacent to the city time display.

- [ ] **Step 12: Wire everything in `App.jsx`**
...
Pass `favorites`, `addFavorite`, `removeFavorite`, `isFavorite` down to `ReverseResults` and `ForwardResult`. Render `<FavoritesList>` at the top of each section with an `onSelect` handler that pre-fills the relevant input.
...
- Date/time override with "Now" reset: Task 11
- Shared `referenceDate` state in App: Task 11
- `useFavorites` hook with localStorage persistence: Task 12
- `useHistory` hook with dedup and 10-entry cap: Task 12
- `FavoritesList` chips with live time: Task 12
